...
import net.Traise.tfs.item.custom.TFSBaseProjectileItem;
...
public class TFSBowTool extends TFSBaseProjectileItem {
    public TFSBowTool(Properties pProperties) {
        super(0, 0, 0, pProperties);
    }
    ...
    public static float getPowerForTime(int pCharge, ItemStack itemStack) {
        float f = (float)pCharge / 20f;
        f = (f * f + f * 2.0F) / 3.0F;
        if (f > 1.0F) {
            f = 1.0F;
        }
    ...
    public boolean canApplyAtEnchantingTable(ItemStack stack, Enchantment enchantment) {
        if (enchantment.category.equals(EnchantmentCategory.BOW)) {
            return true;
        }

...
public class ModelingMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
    ...
    public ModelingMenu(int id, Inventory inv, FriendlyByteBuf extraData, InteractionHand pHand) {
        super(TFSMenuTypes.MODELING_MENU.get(), id);
        this.id = id;
    ...
    public ModelingMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
    ...
    public boolean clickMenuButton(Player pPlayer, int pId) {
        ...
        Optional<ModelingRecipe> recipe = getCurrentRecipe();

        world.playSound(pPlayer, pPlayer.blockPosition(), Blocks.CLAY.getSoundType(world.getBlockState(pPlayer.blockPosition())).getHitSound(), SoundSource.NEUTRAL, 2, 0);
    ...
    private Optional<ModelingRecipe> getCurrentRecipe() {
        BooleanContainer inventory = new BooleanContainer(NonNullList.withSize(25, true));
        for(int i = 0; i < this.map.size(); i++) {
            inventory.setMapSlot(i, this.map.get(i));
        }

        return this.world.getRecipeManager().getRecipeFor(ModelingRecipe.Type.INSTANCE, inventory, world);
    }
        ...
        if (playerIn instanceof ServerPlayer serverPlayer) {
            if (!internal.getStackInSlot(0).isEmpty()) {
                if (!serverPlayer.isCreative()) {
                    serverPlayer.getItemInHand(usedHand).shrink(5);
                }
            }
